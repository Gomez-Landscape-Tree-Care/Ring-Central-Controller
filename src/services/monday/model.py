...
class MondayModel:
    ...
    def update_column_values(self, board_id: int, item_id: str, values: dict,
                             op: str = "") -> str:
        """Overwrite every column in `values` on one item and return the item's id.

        A replace rather than an append: the caller hands over each column's whole
        new value, which is what lets a timeline rebuild drop entries off the old
        end as well as add one to the new.
        """
        data = self.request(
            queries.update_column_values(
                board_id=board_id, item_id=str(item_id), values=values),
            op=op,
        )
